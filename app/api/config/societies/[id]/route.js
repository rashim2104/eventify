...
export async function PUT(req, { params }) {
  ...
  try {
    ...
    await logger(
      user._id,
      ACTION,
      `Society Updated Successfully - ID: ${id}`,
      200
    );
    return NextResponse.json(
      { message: 'Society Updated', society },
      { status: 200 }
    );
  } catch (error) {
    await logger(
      user._id,
      ACTION,
      'Society Update Failed: ' + error.message,
      500
    );
    return NextResponse.json(
      { message: 'An error occurred while updating society.' },
      { status: 500 }
    );
  }
}
...
export async function DELETE(req, { params }) {
  ...
  try {
    ...
    await logger(
      user._id,
      ACTION,
      `Society Deleted Successfully - ID: ${id}`,
      200
    );
    ...
    await logger(
      user._id,
      ACTION,
      'Society Deletion Failed: ' + error.message,
      500
    );
    return NextResponse.json(
      { message: 'An error occurred while deleting society.' },
      { status: 500 }
    );
  }
}
