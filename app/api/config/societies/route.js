...
export async function GET(req) {
  ...
  try {
    ...
    await logger(
      'SYSTEM',
      ACTION,
      'Societies Fetch Failed: ' + error.message,
      500
    );
    return NextResponse.json(
...
export async function POST(req) {
  ...
  try {
    ...
    await logger(
      user._id,
      ACTION,
      `Society Created Successfully - ID: ${society._id}`,
      201
    );
    return NextResponse.json(
      { message: 'Society Created', society },
      { status: 201 }
    );
  } catch (error) {
    if (error.code === 11000) {
      await logger(
        user._id,
        ACTION,
        'Society Creation Failed: Duplicate code',
        400
      );
      return NextResponse.json(
        { message: 'A society with this code already exists.' },
        { status: 400 }
      );
    }
    await logger(
      user._id,
      ACTION,
      'Society Creation Failed: ' + error.message,
      500
    );
    return NextResponse.json(
      { message: 'An error occurred while creating society.' },
      { status: 500 }
    );
  }
}
