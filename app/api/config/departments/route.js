...
export async function GET(req) {
    ...
    try {
        ...
        await logger('SYSTEM', ACTION, 'Departments Fetch Failed: ' + error.message, 500);
        return NextResponse.json({ message: 'An error occurred while fetching departments.' }, { status: 500 });
    }
}
...
export async function POST(req) {
    ...
    try {
        ...
        await logger(user._id, ACTION, `Department Created Successfully - ID: ${department._id}`, 201);
        return NextResponse.json({ message: 'Department Created', department }, { status: 201 });
    } catch (error) {
        if (error.code === 11000) {
            await logger(user._id, ACTION, 'Department Creation Failed: Duplicate code for this college', 400);
            return NextResponse.json({ message: 'A department with this code already exists for this college.' }, { status: 400 });
        }
        await logger(user._id, ACTION, 'Department Creation Failed: ' + error.message, 500);
        return NextResponse.json({ message: 'An error occurred while creating department.' }, { status: 500 });
    }
}
