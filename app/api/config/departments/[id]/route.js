...
export async function PUT(req, { params }) {
    ...
    try {
        ...
        await logger(user._id, ACTION, `Department Updated Successfully - ID: ${id}`, 200);
        return NextResponse.json({ message: 'Department Updated', department }, { status: 200 });
    } catch (error) {
        await logger(user._id, ACTION, 'Department Update Failed: ' + error.message, 500);
        return NextResponse.json({ message: 'An error occurred while updating department.' }, { status: 500 });
    }
}
...
export async function DELETE(req, { params }) {
    ...
    try {
        ...
        await logger(user._id, ACTION, `Department Deleted Successfully - ID: ${id}`, 200);
        return NextResponse.json({ message: 'Department Deleted' }, { status: 200 });
    } catch (error) {
        await logger(user._id, ACTION, 'Department Deletion Failed: ' + error.message, 500);
        return NextResponse.json({ message: 'An error occurred while deleting department.' }, { status: 500 });
    }
}
