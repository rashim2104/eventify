...
export async function PUT(req, { params }) {
  ...
  try {
    ...
    await logger(
      user._id,
      ACTION,
      `College Updated Successfully - ID: ${id}`,
      200
    );
    return NextResponse.json(
      { message: 'College Updated', college },
      { status: 200 }
    );
  } catch (error) {
    await logger(
      user._id,
      ACTION,
      'College Update Failed: ' + error.message,
      500
    );
    return NextResponse.json(
...
export async function DELETE(req, { params }) {
  ...
  try {
    ...
    await logger(
      user._id,
      ACTION,
      `College Deleted Successfully - ID: ${id}`,
      200
    );
    ...
    await logger(
      user._id,
      ACTION,
      'College Deletion Failed: ' + error.message,
      500
    );
    return NextResponse.json(
