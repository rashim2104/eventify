...
export async function GET(req) {
  ...
  try {
    ...
    await logger(
      'SYSTEM',
      ACTION,
      'Colleges Fetch Failed: ' + error.message,
      500
    );
    return NextResponse.json(
...
export async function POST(req) {
  ...
  try {
    ...
    await logger(
      user._id,
      ACTION,
      `College Created Successfully - ID: ${college._id}`,
      201
    );
    return NextResponse.json(
      { message: 'College Created', college },
      { status: 201 }
    );
  } catch (error) {
    if (error.code === 11000) {
      await logger(
        user._id,
        ACTION,
        'College Creation Failed: Duplicate code',
        400
      );
      return NextResponse.json(
    ...
    await logger(
      user._id,
      ACTION,
      'College Creation Failed: ' + error.message,
      500
    );
    return NextResponse.json(
