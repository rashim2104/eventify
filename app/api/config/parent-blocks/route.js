...
export async function GET(req) {
    ...
    try {
        ...
        await logger('SYSTEM', ACTION, 'Parent Blocks Fetch Failed: ' + error.message, 500);
        return NextResponse.json({ message: 'An error occurred while fetching parent blocks.' }, { status: 500 });
    }
}
...
export async function POST(req) {
    ...
    try {
        ...
        await logger(user._id, ACTION, `Parent Block Created Successfully - ID: ${parentBlock._id}`, 201);
        return NextResponse.json({ message: 'Parent Block Created', parentBlock }, { status: 201 });
    } catch (error) {
        if (error.code === 11000) {
            await logger(user._id, ACTION, 'Parent Block Creation Failed: Duplicate name', 400);
            return NextResponse.json({ message: 'A parent block with this name already exists.' }, { status: 400 });
        }
        await logger(user._id, ACTION, 'Parent Block Creation Failed: ' + error.message, 500);
        return NextResponse.json({ message: 'An error occurred while creating parent block.' }, { status: 500 });
    }
}
