...
export async function PUT(req, { params }) {
    ...
    try {
        ...
        await logger(user._id, ACTION, `Club Updated Successfully - ID: ${id}`, 200);
        ...
        return NextResponse.json({ message: 'An error occurred while updating club.' }, { status: 500 });
    }
}
...
export async function DELETE(req, { params }) {
    ...
    try {
        ...
        await logger(user._id, ACTION, `Club Deleted Successfully - ID: ${id}`, 200);
        return NextResponse.json({ message: 'Club Deleted' }, { status: 200 });
    } catch (error) {
        await logger(user._id, ACTION, 'Club Deletion Failed: ' + error.message, 500);
        return NextResponse.json({ message: 'An error occurred while deleting club.' }, { status: 500 });
    }
}
