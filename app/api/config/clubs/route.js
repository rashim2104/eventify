...
export async function GET(req) {
    ...
    try {
        ...
        return NextResponse.json({ message: 'An error occurred while fetching clubs.' }, { status: 500 });
    }
}
...
export async function POST(req) {
    ...
    try {
        ...
        await logger(user._id, ACTION, `Club Created Successfully - ID: ${club._id}`, 201);
        return NextResponse.json({ message: 'Club Created', club }, { status: 201 });
    } catch (error) {
        if (error.code === 11000) {
            await logger(user._id, ACTION, 'Club Creation Failed: Duplicate code', 400);
            return NextResponse.json({ message: 'A club with this code already exists.' }, { status: 400 });
        }
        await logger(user._id, ACTION, 'Club Creation Failed: ' + error.message, 500);
        return NextResponse.json({ message: 'An error occurred while creating club.' }, { status: 500 });
    }
}
