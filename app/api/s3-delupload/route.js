...
export async function POST(req) {
  ...
  try {
    ...
    const oldFileName = formData
      .get('oldFileName')
      .replace('https://eventifys3.s3.ap-south-1.amazonaws.com/', '');
    const id = formData.get('id');
    const action = `eventData.fileUrl.${formData.get('action')}`;

    if (!file) {
      await logger(user._id, ACTION, 'No File Found', 400);
    ...
    const updatedEvent = await Events.findOneAndUpdate(
      { _id: id },
      {
        $set: {
          [action]:
            'https://eventifys3.s3.ap-south-1.amazonaws.com/' + fileName,
        },
      }
    );
    ...
    await logger(user._id, ACTION, 'Operation Failed: ' + error.message, 500);
    return NextResponse.json(
