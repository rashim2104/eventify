...
export async function POST(req) {
    ...
    try {
        ...
        if (!venueId || !startDate || !endDate || !sessions || sessions.length === 0) {
            return NextResponse.json(
        ...
        const existingSet = new Set(
            existingReservations.map(res => `${res.reservationDate}-${res.reservationSession}`)
        );

        // Filter out slots that are already reserved
        const newReservations = requestedSlots
            .filter(slot => !existingSet.has(`${slot.reservationDate}-${slot.reservationSession}`))
            .map(slot => ({
                ...slot,
                venueName,
