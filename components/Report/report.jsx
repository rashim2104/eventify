...
export default function Report() {
  ...
  const handleSearch = async e => {
    ...
    try {
      ...
      const formattedStartDate = new Date(filters.startDate)
        .toISOString()
        .split('T')[0];
  ...
  if (!session?.user?.userType === 'admin') {
    return (
      <div className='grid place-items-center h-screen'>Not Authorized</div>
    );
  }
                ...
                <td className='p-2 border'>
                  ...
                  <div>
                    ...
                    {event.eventData?.EndTime
                      ? new Date(event.eventData.EndTime).toLocaleDateString()
                      : '-'}
                  </div>
                </td>
                <td className='p-2 border'>
