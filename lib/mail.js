...
export const sendMail = async (
  ...
  const baseUrl = process.env.NEXTAUTH_URL || 'https://eventify.sairam.edu.in';
  let sub = '';
  let content = ``;
...
<body style="margin: 0; padding: 0; background-color: ${themeColors.background}; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
  ...
  if (action === 'create') {
    ...
    content = `${headerTemplate}
        ...
        <p style="text-align: center;"><a href="${baseUrl}/events/${eventId}" style="${buttonStyle}">View Event</a></p>
        ${footerTemplate}`;
    ...
    content = `${headerTemplate}
        ...
        <p style="text-align: center;"><a href="${baseUrl}/update/" style="${buttonStyle}">Submit Post-Event Details</a></p>
        ${footerTemplate}`;
  }
  try {
