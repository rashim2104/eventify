...
async function getConfigData() {
  ...
  configCache = {
    ...
    societyCodes: societies.filter(s => s.type === 'professional').map(s => s.code),
    ...
    eventIdTemplates: {
      ...Object.fromEntries(clubs.filter(c => c.eventIdTemplate).map(c => [c.code, c.eventIdTemplate])),
      ...Object.fromEntries(societies.filter(s => s.eventIdTemplate).map(s => [s.code, s.eventIdTemplate])),
      ...Object.fromEntries(departments.filter(d => d.eventIdTemplate).map(d => [d.code, d.eventIdTemplate])),
    },
  };
...
export async function IdGen(eventDoc) {
  ...
  const { clubCodes, societyCodes, ieeeSocietyCodes, deptCodes, eventIdTemplates } = config;
