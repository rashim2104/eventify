...
export async function IdGen(eventDoc) {
  ...
  const firstDayOfMonth = new Date(eventStartDate.getFullYear(), eventStartDate.getMonth(), 1).toISOString().slice(0, 10);
  const lastDayOfMonth = new Date(eventStartDate.getFullYear(), eventStartDate.getMonth() + 1, 0).toISOString().slice(0, 10);
  
  const SIT = "SIT";
  const SEC = "SEC";
  let newId;
  
  if(!clubsShort.includes(eventDoc.dept) && !societies.includes(eventDoc.dept) && !ieeeSocietiesShort.includes(eventDoc.dept) && !departments.includes(eventDoc.dept)){
